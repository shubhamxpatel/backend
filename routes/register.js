...
async function fun(image, req, response) {
    ...
    if (r.length === 0) {
        ...
        await conn.collection("user_image").insertOne(image).then(async res2 => {
                ...
                await conn.collection("login").insertOne({ "user": req.body.email, "pass": req.body.pass, "img": `${id}`, "type": image.mimetype, "file": binary(data1), "logStatus": 0, "watchlist": watchlist, "movie_visited": watchlist })
                    .then(res => {
